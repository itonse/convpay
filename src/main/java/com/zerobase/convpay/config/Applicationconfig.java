...
public class Applicationconfig {   // 사용자(클라이언트는) 요것만 가지고 전체 기능들을 사용해볼 수 있음. 여기서 전체 클래스를 수정 가능  -->더욱 객체지향적으로 변경
    ...
    @Bean
    public static MoneyAdapter moneyAdapter() {   // 머니어댑터는 별도 메서드로 추출
        return new MoneyAdapter();
    }

    @Bean
    public static DiscountBYConvenience discountByConvenience() {   // Bean으로 등록 하려면 private는 안되고, public이여야만 함.
